...
class LSTMModel(nn.Module):
    def __init__(self, feature_dim=512, hidden_dim=256, output_dim=2, num_layers=4):
        super(LSTMModel, self).__init__()
        self.feature_extractor = pvt_v2_b2(num_classes=2)
    ...
    def forward(self, x, is_train=True):
        ...
        if not is_train:
            ...
            # aggregation_feature, importance_scores = self.aggregate(x_feature)
            # bag_out = self.fc(aggregation_feature)
            # # return F.normalize(x_feature, dim=-1), x_logits, F.normalize(h_lstm[:, -1, :]), bag_out
            # return F.normalize(x_feature, dim=-1), x_logits, F.normalize(aggregation_feature, dim=-1), bag_out
            # # return x_feature, x_logits, h_lstm[:, -1, :], lstm_out
...
class LSTMModelEnd2End(nn.Module):
    def __init__(self, feature_dim=512, hidden_dim=256, output_dim=2, num_layers=4):
        super(LSTMModelEnd2End, self).__init__()
        self.feature_extractor = pvt_v2_b2(num_classes=2)
        self.lstm = nn.LSTM(feature_dim, hidden_dim, num_layers, batch_first=True)
    ...
    def forward(self, x, is_train=True):
        ...
        _, indices = torch.topk(x_softmax[:, :, 1], 7, dim=1)
        top_features = x_feature.gather(1, indices.unsqueeze(-1).expand(-1, -1, 512))
        top_logits = x_logits.gather(1, indices.unsqueeze(-1).expand(-1, -1, 2))
        top_softmax = x_softmax.gather(1, indices.unsqueeze(-1).expand(-1, -1, 2))

        h_lstm, _ = self.lstm(top_features)
        lstm_out = self.fc(h_lstm[:, -1, :])  # 使用最后一个LSTM cell的输出

        return x_logits, x_softmax, top_logits, top_softmax, lstm_out,\
            F.normalize(x_feature, dim=-1), F.normalize(top_features, dim=-1), F.normalize(h_lstm[:, -1, :], dim=-1)
