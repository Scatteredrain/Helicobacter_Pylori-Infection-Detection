...
class MILModel(nn.Module):
    def __init__(self, feature_dim=512, hidden_dim=512, output_dim=2, num_layers=4, instance_batch_size=20, pvt_add_cls_head=False, add_center_loss=False):
        ...
        if add_center_loss:
            self.center_loss = CenterLoss(num_classes=2, feat_dim=hidden_dim, use_gpu=True)

    def forward(self, x_feature):
        b,k,e = x_feature.shape
        ...
        return out_aggregation, importance_scores #,x_feature,x_logits,x_softmax        
...
class PolypPVT_CLS(nn.Module):
    def __init__(self, feature_dim=512, output_dim=2, pvt_add_cls_head=False):
        super(PolypPVT_CLS, self).__init__()
        self.pvt_add_cls_head = pvt_add_cls_head
        self.feature_extractor = pvt_v2_b2(num_classes=2, add_cls_head=pvt_add_cls_head)
        self.cls_head = nn.Linear(feature_dim, output_dim)

    def forward(self, x):
        if self.pvt_add_cls_head:
            x_features, x_logits, x_softmax = self.feature_extractor(x)
            return x_logits, x_softmax
        else:
            x_features = self.feature_extractor(x)
            x_logits = self.cls_head(x_features)
            return x_logits